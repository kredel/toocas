...
class UnivariatePolynomial[C, @specialized(Int, Long) N](val ring: Field[C], val pp: PowerProduct[N], val module: Module[Element[C, N]])(implicit val ordering: Ordering[N], val cm: ClassManifest[Element[C, N]]) extends TreePolynomial[Element[C, N], C, N] with scas.polynomial.ufd.UnivariatePolynomial[Element[C, N], C, N] {
  def this(ring: Field[C], pp: PowerProduct[N])(implicit ordering: Ordering[N], cm: ClassManifest[Element[C, N]]) = this(ring, pp, Module("e", 0, null))
...
object UnivariatePolynomial {
  def apply[C, @specialized(Int, Long) N](ring: Field[C], pp: PowerProduct[N])(implicit ordering: Ordering[N]) = new UnivariatePolynomial(ring, pp)
