...
class PolynomialWithSimpleGCD[C <: UniqueFactorizationDomain[C], @specialized(Int, Long) N](val ring: C, val pp: PowerProduct[N])(implicit val ordering: Ordering[N]) extends scas.polynomial.ufd.PolynomialWithSimpleGCD[PolynomialWithSimpleGCD, C, N] with TreePolynomial[PolynomialWithSimpleGCD[C, N], C, N] {
